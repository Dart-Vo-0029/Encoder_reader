...
int main(void)
{
    ...
    while(1)
    {
        ...
        {
            rx_ready = 0;
            int value = atoi((char*)rx_msg_buffer);
            ...
            if(value > 0) state = ST_RUN_FORWARD;
            else if(value < 0) state = ST_RUN_REVERSE;
            else state = ST_STOP;
        }
...
void SysTick_Handler(void)
{
    ...
    measured_rpm = (int)(diff * 30000.0f / 200.0f);
    /* 3000 = 60s / 0.02s (50Hz encoder sampling) adjust as needed */

    if(state != ST_STOP)
    {
        /* ==== PID ==== */
        int error = target_rpm - measured_rpm;
